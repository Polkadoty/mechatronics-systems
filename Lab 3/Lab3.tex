...
Table 1 contains the voltage data at various positions along the chord.  It is then converted into distance using the calibration equation.  The deflection at the Leading Edge and Trailing Edge is used to calculate the angle of twist with Eq. XX %arctan eq
and shear center of the wing.  Plotting the angle of twist against the X position gives a linear representation where the intersect at 0 radians will be the shear center.  This is shown in figure X %add graph here
...
For this section, \(X_j\) was taken as 27.5 in, or 69.85 \(cm\), the following values were calculated using equations %Deflection equation, Moment equation, Torque Equation
