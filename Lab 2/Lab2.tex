...
Temperature is converted to Kelvin for calculation purposes. The $1$,$2$,and $3$ subscripts represent the three environments (in no particular order).
...
The time constant, (PUT TAU HERE),  is defined as the time it takes for the system to change from its original state by 63\%.  % T(t)=(Tf=To)(1-exp(-t/tau))+To then find T at t=tau
